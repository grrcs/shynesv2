...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-2xl font-bold text-primary dark:text-white">Tambah Produk Baru</h2>
                <p class="text-sm text-secondary dark:text-gray-400">Masukkan detail barang dagangan Anda.</p>
            </div>
        ...
        <div class="bg-white dark:bg-primary border-thin dark:border-gray-800 transition-colors p-8">
            <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" class="w-full px-4 py-3 bg-transparent border-thin dark:border-gray-800 text-primary dark:text-white outline-none transition-colors focus:ring-2 focus:ring-black outline-none" placeholder="Contoh: Oversized Tee Black" value="{{ old('title') }}">
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="price" class="w-full px-4 py-3 bg-transparent border-thin dark:border-gray-800 text-primary dark:text-white outline-none transition-colors focus:ring-2 focus:ring-black outline-none" placeholder="150000" value="{{ old('price') }}">
                                @error('price') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-secondary dark:text-gray-300 mb-2">Stok</label>
                                <input type="number" name="stock" class="w-full px-4 py-3 bg-transparent border-thin dark:border-gray-800 text-primary dark:text-white outline-none transition-colors focus:ring-2 focus:ring-black outline-none" placeholder="100" value="{{ old('stock') }}">
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-secondary dark:text-gray-300 mb-2">Link E-Commerce (Opsional)</label>
                            <input type="text" name="link_shopee" class="w-full px-4 py-3 bg-transparent border-thin dark:border-gray-800 text-primary dark:text-white outline-none transition-colors focus:ring-2 focus:ring-black outline-none" placeholder="https://shopee.co.id/..." value="{{ old('link_shopee') }}">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-secondary dark:text-gray-300 mb-2">Status</label>
                            <select name="status" class="w-full px-4 py-3 bg-transparent border-thin dark:border-gray-800 text-primary dark:text-white outline-none transition-colors focus:ring-2 focus:ring-black outline-none dark:bg-primary mb-6">
                        ...
                        <div class="border border-thin dark:border-gray-800 p-4 rounded-xl space-y-4">
                            <label class="flex items-center space-x-3 cursor-pointer">
                                <input type="checkbox" name="is_discount_active" value="1" {{ old('is_discount_active') ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-black focus:ring-black">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="discount_price" class="w-full px-3 py-2 bg-transparent border-thin dark:border-gray-800 text-primary dark:text-white outline-none transition-colors focus:ring-2 focus:ring-black" placeholder="Cth: 90000" value="{{ old('discount_price') }}">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-secondary dark:text-gray-300 mb-2">Kuota Max (Item)</label>
                                    <input type="number" name="discount_limit" class="w-full px-3 py-2 bg-transparent border-thin dark:border-gray-800 text-primary dark:text-white outline-none transition-colors focus:ring-2 focus:ring-black" placeholder="Cth: 5" value="{{ old('discount_limit') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="content" rows="6" class="w-full px-4 py-3 bg-transparent border-thin dark:border-gray-800 text-primary dark:text-white outline-none transition-colors focus:ring-2 focus:ring-black outline-none resize-y" placeholder="Tuliskan deskripsi produk di sini...">{{ old('description') }}</textarea>
                    @error('description') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="pt-4 border-t border-gray-100">
                    <button type="submit" class="w-full py-3 bg-primary text-white dark:bg-white dark:text-primary font-bold rounded-lg hover:bg-black dark:bg-gray-200 transition-all">SIMPAN PRODUK</button>
                </div>
            </form>
        </div>
    </div>
