...
<head>
    ...
    <title>Tambah Produk - Shyness OS</title>
    ...
    <nav class="bg-white border-b border-gray-200 px-4 py-3 shadow-sm sticky top-0 z-20">
        <div class="max-w-7xl mx-auto flex justify-between items-center">
            ...
            <div class="flex items-center gap-6">
                <a href="{{ route('posts.index') }}" class="text-sm font-medium text-gray-500 hover:text-gray-900">Postingan</a>
                <a href="{{ route('products.index') }}" class="text-sm font-bold text-gray-900 border-b-2 border-gray-900 pb-0.5">Produk</a>
                <a href="{{ route('categories.index') }}" class="text-sm font-medium text-gray-500 hover:text-gray-900">Kategori</a>
    ...
    <div class="max-w-4xl mx-auto px-4 py-10">
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500">Masukkan detail barang dagangan Anda.</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow border border-gray-200 p-8">
            <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-black outline-none" placeholder="Contoh: Oversized Tee Black" value="{{ old('title') }}">
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="price" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-black outline-none" placeholder="150000" value="{{ old('price') }}">
                                @error('price') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">Stok</label>
                                <input type="number" name="stock" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-black outline-none" placeholder="100" value="{{ old('stock') }}">
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Link E-Commerce (Opsional)</label>
                            <input type="text" name="link_shopee" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-black outline-none" placeholder="https://shopee.co.id/..." value="{{ old('link_shopee') }}">
                        </div>
                        <div>
                ...
                <div>
                    ...
                    <textarea name="description" id="content" rows="4" class="w-full border rounded-lg">{{ old('description') }}</textarea>
                    @error('description') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="pt-4 border-t border-gray-100">
                    <button type="submit" class="w-full py-3 bg-gray-900 text-white font-bold rounded-lg hover:bg-black transition-all">SIMPAN PRODUK</button>
                </div>
            </form>
        </div>
    </div>
