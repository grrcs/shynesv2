...
@section('title', 'Tambah Alamat - Shyness')
...
<div class="max-w-2xl mx-auto">
    <div class="mb-10 border-b border-thin pb-6 border-gray-200 dark:border-gray-800 transition-colors">
        <div class="flex items-center gap-4 mb-2">
            ...
            <h1 class="text-3xl font-serif font-medium text-primary dark:text-white transition-colors">Tambah Alamat</h1>
        </div>
        <p class="text-xs tracking-widest uppercase text-secondary ml-8">Tambahkan alamat pengiriman baru Anda</p>
    </div>

    <form action="{{ route('addresses.store') }}" method="POST" class="bg-white dark:bg-primary border border-thin border-gray-200 dark:border-gray-800 rounded-xl p-8 shadow-sm transition-colors">
        @csrf
        ...
        <div class="mb-8">
            <label class="flex items-center space-x-3 cursor-pointer group">
                <input type="checkbox" name="is_primary" value="1" {{ old('is_primary') ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-black focus:ring-black dark:border-gray-700 dark:bg-[#151515] dark:checked:bg-white dark:focus:ring-white transition-all cursor-pointer">
                <div>
